...
if (!function_exists('saweeklyreportIsPdfDocumentModel')) {
	/**
	 * Check if a document model key is a PHP/TCPDF model.
	 *
	 * @param	string	$model	Document model key
	 * @return	bool
	 */
	function saweeklyreportIsPdfDocumentModel($model)
	{
		return (bool) preg_match('/^pdf_[A-Za-z0-9_]+$/', (string) $model);
	}
}

if (!function_exists('saweeklyreportGetPptxTemplatePath')) {
	/**
	 * Return path of a safe PPTX template model.
	 *
	 * @param	string	$model	PPTX model key
	 * @return	string
	 */
	function saweeklyreportGetPptxTemplatePath($model)
	{
		if (!preg_match('/^[A-Za-z0-9_-]+$/', (string) $model)) {
			return '';
		}
...
if (!function_exists('saweeklyreportGetDocumentModelType')) {
	/**
	 * Return document model type handled by the weekly report generator.
	 *
	 * @param	string	$model	Document model key
	 * @return	string	'pdf', 'pptx' or ''
	 */
	function saweeklyreportGetDocumentModelType($model)
	{
		if (saweeklyreportIsPdfDocumentModel($model)) {
			return 'pdf';
		}
		if (saweeklyreportGetPptxTemplatePath($model) !== '') {
			return 'pptx';
		}
...
if (in_array($action, array('setmod', 'updateMask', 'set', 'del', 'setdoc'), true)) {
	...
	if ($action === 'setmod') {
		...
		if ($modeltype === '' || addDocumentModel($value, $documenttype, $label, $description) <= 0) {
			$error++;
		}
	} elseif ($action === 'del') {
		$modeltype = saweeklyreportGetDocumentModelType($value);
...
foreach ($dirmodels as $reldir) {
	foreach (array('', '/doc') as $valdir) {
		...
		foreach ($filelist as $file) {
			...
			$documentmodels[$name] = array(
				'name' => $name,
				'label' => $label,
				'description' => $description,
				'scan' => $scan,
				'type' => 'pdf',
				'path' => preg_replace('/^\//', '', $realpath).'/'.$file,
				'page_width' => isset($module->page_largeur) ? (string) $module->page_largeur : '',
				'page_height' => isset($module->page_hauteur) ? (string) $module->page_hauteur : '',
			);
		}
	}
}
...
$pptxdirs = array(
	dol_buildpath('/saweeklyreport/doctemplates/saweeklyreport', 0) => 'doctemplates/saweeklyreport',
	DOL_DATA_ROOT.'/doctemplates/saweeklyreport' => 'DOL_DATA_ROOT/doctemplates/saweeklyreport',
);
foreach ($pptxdirs as $dir => $displaydir) {
	if (!is_dir($dir)) {
		continue;
	}
	$handle = opendir($dir);
	...
	sort($filelist);
	foreach ($filelist as $file) {
		if (!preg_match('/^([A-Za-z0-9_-]+)\.pptx$/', $file, $matches)) {
			continue;
		}
		$name = $matches[1];
		if (isset($documentmodels[$name])) {
			continue;
		}
		$label = ($name === 'weekly_report_standard') ? $langs->trans('WeeklyReportPptxStandardModel') : $name;
		$documentmodels[$name] = array(
			'name' => $name,
			'label' => $label,
			'description' => $langs->trans('WeeklyReportPptxModel'),
			'scan' => '',
			'type' => 'pptx',
			'path' => $displaydir.'/'.$file,
			'page_width' => '',
			'page_height' => '',
		);
	}
...
ksort($documentmodels);
foreach ($documentmodels as $modeldata) {
	$documentmodelfound++;
	...
	if ($modeltype === 'pdf') {
		$htmltooltip .= '<br>'.$langs->trans('Width').'/'.$langs->trans('Height').': '.dol_escape_htmltag((string) $modeldata['page_width']).'/'.dol_escape_htmltag((string) $modeldata['page_height']);
	}
	$htmltooltip .= '<br>'.$langs->trans('Path').': '.dol_escape_htmltag((string) $modeldata['path']);
	print '<td class="center">'.$form->textwithpicto('', $htmltooltip, 1, 'info').'</td>';
	print '</tr>';
}
